...
{
    internal    Texture2D   texture2D;  // can be hidden in Inspector with [UiHide]
    ...
    static void CopyValue(in TileSet source, ref TileSet target, in CopyContext context) {
        target.texture2D    = source.texture2D;
...
struct GameInfo : IComponent
{
    internal MonoGame.EcGuiLab.Game1    game;
    internal MouseState                 mouse       => Mouse.GetState();
    internal KeyboardState              keyboard    => Keyboard.GetState();
    internal GamePadState               gamePadOne  => GamePad.GetState(Microsoft.Xna.Framework.PlayerIndex.One);
}
