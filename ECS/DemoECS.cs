...
internal static class DemoECS
{
    ...
    /// </summary>
    /// <remarks>
    /// The game parameter is passed to load textures.
    /// </remarks>
    internal static void CreateEntityStore(Game1 game)
    {
        ...
        tileSetEntity7.AddComponent(new TileSet { spriteWidth = 64, spriteHeight = 64, texture = "tiny-wonder-rpg-icons", texture2D = game.LoadTexture2D("tiny-wonder-rpg-icons") });

        var tileSetEntity8 = store.CreateEntity(8);
        tileSetEntity8.AddComponent(new EntityName("world tileset"));
        tileSetEntity8.AddComponent(new TileSet { spriteWidth = 64, spriteHeight = 64, texture = "world_tileset", texture2D = game.LoadTexture2D("world_tileset") });
        
        var monoGamEntity = store.CreateEntity(9);
        monoGamEntity.Add(new EntityName("GameInfo"), new GameInfo { game = game });
...
#pragma warning disable CS0162 // Unreachable code detected
    ...
    /// The game parameter is passed to enable drawing sprites in EcGui using a custom <see cref="SpriteDrawer"/>.
    /// </summary>
    internal static void CustomizeEcGui(Game1 game)
    {
        // --- register custom TypeDrawer's on startup
        TypeDrawers.Register(game.guiRenderer);
        
        // --- add event Handler: Update the texture2D when texture name is changed via EcGui 
        EcGui.Setup.AddOnMemberChanged((ref TileSet value, Entity _, string _, in TileSet _) => {
            value.texture2D = null; // loading texture may throw exception
            value.texture2D = game.LoadTexture2D(value.texture);
            game.guiRenderer.NotifyTexturesChanged();
        });
